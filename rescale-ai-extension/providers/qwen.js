...
const ZSProvider = (() => {
  ...
  // Scope to the site's composer only; skip Rescale AI's own injected textarea
  ...
  // of writing Rescale AI commands as text - it then answers in plain text or
    ...
    dd.querySelectorAll(".ant-select-item-option").forEach((opt) => {
      ...
      if (UNSTABLE_MODE_RE.test(label)) {
        if (!existing) {
          ...
          b.title =
            "In this mode Qwen tends to run its OWN tools instead of the Rescale AI " +
