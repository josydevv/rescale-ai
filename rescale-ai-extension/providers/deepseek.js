...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match Rescale AI's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole Rescale AI bar INTO the
  ...
  function enforceComposer(reason) {
    ...
    // off) without Rescale AI reverting their choice every frame.
  ...
  //   isStarted():bool        - a Rescale AI session exists in this chat
